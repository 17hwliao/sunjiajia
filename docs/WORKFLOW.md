...
执事 AI 可以把 `READY` 任务拆分为带依赖关系的子任务图。无依赖且文件所有权不重叠的子任务可以并行；存在公共接口、共享文件或未完成依赖时必须等待或显式上报冲突。

每个子 Agent 都必须输出可审计的证据（改动、测试、限制和风险）。只有独立测试与安全复核通过的候选改动，才能进入待人工确认的合并阶段。任何子 Agent 都不能自行推送、合并、覆盖他人工作或获得系统自动化权限。
